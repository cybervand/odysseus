...
def setup_session_routes(
    ...
    # get_history lived here until the #5929 sync: it shadowed the canonical
    # paged /api/history/{session_id} (routes/history) by router order, so
    # pagination silently never applied. The doc 013 checkpoint promotion and
    # doc 014 feed-log overlay it carried now run inside the canonical
    # endpoint (page-aware overlay: src/feed_log.overlay_rows).
