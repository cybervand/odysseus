...
FUNCTION_TOOL_SCHEMAS = [
    {
        ...
        "function": {
            "name": "bash",
            "description": "Run a shell command (full access). Prefer a dedicated tool whenever one fits the job (reading, writing, editing, searching, or listing files); use bash only for what no dedicated tool covers (installs, git, builds, running programs, system info). Do NOT create or edit files via bash redirects/heredocs/sed -- use the dedicated file tools. There is NO terminal attached: interactive commands (setup wizards, --init prompts, editors, pagers) hang or fail -- always pass non-interactive flags (--yes, --defaults, -y).",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "manage_server",
            "description": "Run and manage long-lived servers (web apps, APIs, dev servers). These are YOUR servers — edit their code freely. NEVER start a server with bash — it never exits and blocks everything. Ports are ASSIGNED by Odysseus from its reserved range and exported to your server process as the PORT env var: apps read os.environ['PORT']; programs that take the port as an argument write $PORT in the command (e.g. 'python3 -m http.server $PORT'). Never hardcode a port and never pass one here; PORT is not visible from bash, only inside your server. Actions: start (name+command+cwd), stop, restart (relaunches; pass a new command to replace it), status, logs, list, query (HTTP-probe your server: path/method/body — works even without bash), adopt (attach a server from another chat to this one), remove (delete registry entry). CRITICAL: after editing server code you MUST restart — a running process does not see file edits.",
            "parameters": {
                "type": "object",
                "properties": {
