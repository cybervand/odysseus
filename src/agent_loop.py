...
async def _run_verifier_subagent(
    ...
    *, endpoint_url: str, model: str, headers: dict,
    ...
    prompt = (
        ...
        "3. A requirement grounded in an attached image, photo, or other "
        "visual content is one you CANNOT check — you never see pixels. If "
        "the record shows an answer or action plausibly addressing it, count "
        "it MET; NEVER substitute your own guess about what the image "
        "contains, and never fail a visual claim for lacking proof only "
        "pixels could give.\n"
        "4. A tool error or empty output that was never addressed afterwards is "
        "a failure.\n\n"
