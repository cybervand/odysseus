...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_server": "Run and manage the user's long-lived named servers (web apps, APIs, dev servers): start/stop/restart/status/logs/list plus query (HTTP-probe your own server, works without bash), adopt (attach a server from another chat to this one), remove. NEVER start a server with plain bash — it never exits. Ports are ASSIGNED by Odysseus and exported as the PORT env var; apps must read it, not hardcode a port. After editing server code you MUST restart — a running process does not see file edits. Use for 'start/stop/restart my server', 'is my app running', 'show the server logs'.",
    "manage_framework": "Install/manage frameworks and CLI tools in the SHARED persistent tier that survives app updates and is available to every chat (vite, tailwindcss, typescript, flask...). Actions: install, upgrade, remove, list, info. NOT for project-local deps — those are plain npm/pip installs inside the project. Use for 'install tailwind', 'do we have vite', 'upgrade typescript everywhere'.",
    "find_images": "Find VERIFIED image URLs for a subject (web image search + Wikimedia Commons), already checked live (HTTP 200) — use the returned URLs directly in HTML/markdown. Always use this to get real images; never invent image URLs and never scrape image pages by hand. Batch subjects (count up to 5 per call) instead of calling once per page section. Use for 'find a picture of X', 'add real images to the page'.",
}
