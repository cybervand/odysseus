...
// Strategy:
//   - HTML (navigation): network-first, cache fallback for offline. It was
//     stale-while-revalidate, but that guarantees one broken visit after
//     every deploy that changes both HTML and JS: the refresh served OLD
//     cached HTML with NEW network-first JS, and any markup/JS contract
//     change (id renames etc.) left dead controls until a second reload
//     (bit us 2026-08-11: command-menu verifier switch). On a LAN the
//     network-first cost is milliseconds; correctness wins.
...
const CACHE_NAME = 'odysseus-v377-nav-network-first';
...
self.addEventListener('fetch', (e) => {
  ...
  // HTML navigation: network-first so the shell always matches the JS the
  // network-first handler below will serve (see header comment) — but ONLY
  // for the SPA root. Other navigations (e.g. a deep-linked /static/*.html
  // page) must go to the network/static handlers below; otherwise every
  // navigation was served the app index, replacing the page the user
  // actually asked for.
